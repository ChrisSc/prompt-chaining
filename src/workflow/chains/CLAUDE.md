...
3. Initialize ChatAnthropic with model, temperature, token limits, and request_id propagation:
   ...
   ```
4. Build message list with SystemMessage (prompt) and HumanMessage (input)
5. Call `await llm.ainvoke(messages)` to get response
6. Parse JSON response into output model (AnalysisOutput, ProcessOutput, SynthesisOutput)
...
Exceptions are logged with full context (error type, truncated response, step name) then re-raised so LangGraph conditional edges route to error handling.
